...
#define  RECONSTRUCT_STATE 1
#define  RECONSTRUCT_COMPLETE 2
...
struct ssd_info{ 
	...
	struct	visit_count visit_table[1024];	//visit table用于记录数据的访问次数
	unsigned int visit_table_size;	//1024
	unsigned int visit_index;
	unsigned int used_visit_table_count;
	struct restore_node *restore_list;
	unsigned int node_size; //每个node跨越的lpn范围
	struct restore_node *list_head,*list_tail;
	unsigned int list_count,max_list_count;

	int state;	//继承raid 的state状态 0:normal 1:reconstrut 2:reconstruct complete
	struct sub_request *restore_req;
};
...
struct sub_request{
	...
	struct sub_request *next_pre_read;
	struct sub_request *update;       //因为在写操作中存在更新操作，因为在动态分配方式中无法使用copyback操作，需要将原来的页读出后才能进行写操作，所以，将因更新产生的读操作挂在这个指针上

	struct sub_request *next_rec;
	struct sub_request *trig_req;
	int rec_flag;						//分读写：读的recflag为一表明这个读引起了reconstruct操作，写表明这是rec的写操作
	int on_pocess;

	int RMW_flag;	//-1：表示这是一个rcw，1：表示这是一个RMW。0：表示该请求不在parity盘上
	int priority;	//0:正常；-1：低优先级；
};
...
int initialize_restore_list(struct ssd_info* ssd);