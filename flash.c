...
Yang Hu	        2009/09/copy25	      1.0		    Creat SSDsim       yanghu@foxmail.com
		...
		****************************************/
		switch(ssd->parameter->dynamic_allocation)
		{
			case 0:
			{
				...
				if ((update!=NULL)&&(sub_req->update==NULL))	//加上判断update判断条件，为了避免在distribute之前预读parity已经挂载了update
				{
...
{
	struct sub_request * sub=NULL,* p=NULL,*tmp=NULL,*tmp1;
	int pre_read_complete_flag;
	int exist_normal_req_flag;

	exist_normal_req_flag=0;
	if ((ssd->parameter->allocation_scheme==0)&&(ssd->parameter->dynamic_allocation==0))    /*是完全的动态分配*/
	{
		...
		while(sub!=NULL)        							
		{
			if (exist_normal_req_flag==1&&sub->priority==-1)
			{
				p=sub;
				sub=sub->next_node;
				continue;
			}
			pre_read_complete_flag=1;
			if(sub->current_state==SR_WAIT)								
			{
				//先检查是否有相应预读请求，相应于读请求完成了才能开始处理
				tmp=sub->next_pre_read;
				while (tmp!=NULL)
				{	
					pre_read_complete_flag=1;
					if (tmp->current_state==SR_COMPLETE||((tmp->next_state==SR_COMPLETE)&&(tmp->next_state_predict_time<=ssd->current_time)))
					{
						pre_read_complete_flag=1;
						tmp=tmp->next_pre_read;
					}
					else
					{
						pre_read_complete_flag=0;
						break;
					}
				}
				if (pre_read_complete_flag==1)
				{		
...
int services_2_r_wait(struct ssd_info * ssd,unsigned int channel,unsigned int * channel_busy_flag, unsigned int * change_current_time_flag)
{
	...
	struct sub_request * sub=NULL, * p=NULL, *tmp=NULL;
	struct sub_request * sub_twoplane_one=NULL, * sub_twoplane_two=NULL;
	struct sub_request * sub_interleave_one=NULL, * sub_interleave_two=NULL;
	int exist_normal_req_flag;
	...
	*******************************/
	if (((ssd->parameter->advanced_commands&AD_INTERLEAVE)!=AD_INTERLEAVE)&&((ssd->parameter->advanced_commands&AD_TWOPLANE_READ)!=AD_TWOPLANE_READ))
	{
		...
		{		
			if(sub->current_state==SR_WAIT)									
			{	 
				//若存在高优先级的请求则低优先级的请求暂不处理
				if (exist_normal_req_flag==1&&sub->priority==-1&&sub->next_node!=NULL)
				{
					sub=sub->next_node;
					continue;
				}
