...
{
	//ssd->restore_list = (struct restore_node*)malloc(sizeof(struct restore_node));
	//alloc_assert(ssd->restore_list,"ssd->restore_list");
	//memset(ssd->restore_list,0,sizeof(struct restore_node));


	//ssd->restore_list->id = 0;
	//ssd->restore_list->next_node = ssd->restore_list;
	//ssd->restore_list->pre_node = ssd->restore_list;
	//ssd->restore_list->state = 0;
	ssd->restore_list=NULL;
	ssd->max_list_count=1024;

	return 0;
}


int initialize_visit_table(struct ssd_info *ssd)
{
	int id;
	ssd->visit_table_size=1024;
	ssd->node_size=32;
	for (id=0;id<512;id++)
	{
		ssd->visit_table[id].count=-1;
	}	
}
